...
PACKAGES=$(ls ${ROOT} -l | awk '{print $9}' | sed 's/\(.*\)\..*/\1/')
...
echo $URL
echo ${CIRCLE_TOKEN}
HTTP_RESPONSE=$(curl -s -u 355ea2bff28c6dc5b363d3575768b17a3c8b0d22: -o response.txt -w "%{http_code}" -X POST --header "Content-Type: application/json" -d "$DATA" $URL)
